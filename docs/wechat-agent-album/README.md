...
- 图片：`images/` 目录，共 847 张（731 张正文图片、116 张背景及附加资源）；所有图片引用均已改为相对路径
- 清单：`index.json` 记录标题、原文链接、发布时间、正文长度和本地图片映射
- 维护：`localize-assets.ps1` 可重新检查并本地化微信 CDN 图片引用

直接用浏览器打开任意编号的 HTML 文件即可阅读，不依赖网络连接。
